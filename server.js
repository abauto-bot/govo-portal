...
async function fetchTrackingResults({ id = '', phone = '', type = '', code = '' }) {
  ...
  if (type !== 'service') {
    if (code && !String(code).toUpperCase().startsWith('SRV-')) out.orders = (await pool.query(`SELECT * FROM govo_orders WHERE order_code=$1 ORDER BY id DESC LIMIT 10`, [code])).rows;
  ...
  if (type !== 'order') {
    if (code) out.services = (await pool.query(`SELECT * FROM govo_service_requests WHERE request_code=$1 ORDER BY id DESC LIMIT 10`, [code])).rows;
...
app.get('/admin/service-requests', async (req, res, next) => {
  try {
    ...
    const stat = (label, value) => `<div class="stat"><div class="label">${esc(label)}</div><div class="value">${esc(value || 0)}</div></div>`;
    const opt = (v, label) => `<option value="${v}" ${status === v ? 'selected' : ''}>${label}</option>`;
    res.send(page('Service Requests', `<section class="card app-hero"><h1>Service Requests</h1><p>Review bookings, assign providers, and monitor service progress.</p><div class="actions"><a class="btn secondary" href="/admin/os">Admin OS</a><a class="btn secondary" href="/admin/orders">Order Dispatch</a><a class="btn secondary" href="/admin/providers">Providers</a></div></section><section class="grid">${stat('Total', c.total)}${stat('New', c.new)}${stat('Confirmed', c.confirmed)}${stat('Assigned', c.assigned)}${stat('In Progress', c.in_progress)}${stat('Completed', c.completed)}${stat('Cancelled', c.cancelled)}</section><section class="card"><h2>Filters</h2><form class="filters" method="GET" action="/admin/service-requests"><input name="q" value="${esc(q)}" placeholder="Search code, customer, provider, service, problem, area"><select name="status"><option value="all">All</option>${opt('new','New')}${opt('confirmed','Confirmed')}${opt('assigned','Assigned')}${opt('in_progress','In Progress')}${opt('completed','Completed')}${opt('cancelled','Cancelled')}</select><button>Search</button></form></section><section class="grid two">${column('new','New')}${column('confirmed','Confirmed')}${column('assigned','Assigned')}${column('in_progress','In Progress')}${column('completed','Completed')}${column('cancelled','Cancelled')}</section>`, 'admin'));
  } catch (e) { next(e); }
});
