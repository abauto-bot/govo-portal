...
async function ensureSchema() {
  ...
  for (const col of ['shop_name TEXT', 'merchant_phone TEXT', 'customer_name TEXT', 'customer_phone TEXT', 'pickup_location TEXT', 'drop_location TEXT', 'item_details TEXT', 'note TEXT', 'preferred_time TEXT', 'customer_note TEXT', "status TEXT DEFAULT 'pending'", 'admin_note TEXT', 'merchant_note TEXT', 'rider_id INT', 'rider_name TEXT', 'rider_phone TEXT', 'assigned_rider_id INT', 'assigned_rider_name TEXT', 'assigned_rider_phone TEXT', 'rider_note TEXT', 'merchant_lead_id INTEGER', "order_type TEXT DEFAULT 'delivery'", 'created_at TIMESTAMPTZ DEFAULT NOW()', 'updated_at TIMESTAMPTZ DEFAULT NOW()']) await add('govo_orders', col);
...
app.get('/admin/orders', async (req, res, next) => {
  try {
    ...
    if (q) {
      ...
      where.push(`LOWER(COALESCE(shop_name,'') || ' ' || COALESCE(merchant_phone,'') || ' ' || COALESCE(customer_name,'') || ' ' || COALESCE(customer_phone,'') || ' ' || COALESCE(pickup_location,'') || ' ' || COALESCE(drop_location,'') || ' ' || COALESCE(item_details,'') || ' ' || COALESCE(rider_name,'') || ' ' || COALESCE(rider_phone,'') || ' ' || COALESCE(assigned_rider_name,'') || ' ' || COALESCE(assigned_rider_phone,'')) LIKE $${params.length}`);
    }
    const [orders, riders, counts] = await Promise.all([
      pool.query(`SELECT id, shop_name, merchant_phone, customer_name, customer_phone, pickup_location, drop_location, item_details, note, preferred_time, customer_note, COALESCE(status,'pending') AS status, admin_note, merchant_note, rider_id, rider_name, rider_phone, assigned_rider_id, assigned_rider_name, assigned_rider_phone, rider_note, created_at, updated_at FROM govo_orders ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY id DESC LIMIT 150`, params),
...
function progressStage(type, status) {
  ...
  if (type === 'order') {
    ...
    if (['rejected', 'failed', 'cancelled'].includes(s)) return 5;
    return 1;
  }
  if (['completed', 'delivered'].includes(s)) return 4;
  if (['assigned', 'working', 'ready'].includes(s)) return 3;
  if (s === 'accepted') return 2;
...
function timelineHtml(type, status) {
  ...
  const labels = type === 'order' ? ['Submitted', 'Admin Review', 'Rider Assigned', 'Picked Up', 'Delivered'] : ['Submitted', 'Accepted', 'Assigned / Working', 'Completed'];
